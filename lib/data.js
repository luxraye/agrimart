// ─── Static product data ─────────────────────────────────────────────────────
// Everything in this file is *intentionally* static (see addendum §"WHAT STAYS
// STATIC"). All intelligence values (supply signals, risk scores, field intel)
// live in the data pipeline — the only intelligence values kept here are the
// clearly-labelled BASELINE_* fallbacks used when live sources are unavailable.
...
export const CROP_KEYS = CROPS.map((c) => c.value);

// FAO FAOSTAT item codes (area code 20 = Botswana)
export const FAO_CROP_CODES = {
  tomato: 388, cabbage: 358, onion: 403, potato: 116,
  carrot: 426, sorghum: 83, beans: 176, spinach: 373,
};
export const FAO_AREA_BOTSWANA = 20;
...
// ─── Baseline supply signals ─────────────────────────────────────────────────
// FALLBACK: replace when ≥3 farmer submissions exist for a district+crop.
// Used only when the planting_intentions table has insufficient live data —
// the UI must render the "Based on baseline data" badge whenever these values
// are displayed.

export const BASELINE_SUPPLY_SIGNALS = {
  tomato: {
  ...
  cabbage: {
    ...
    central:   { status: "go",      supply: 30, demand: 75, label: "Demand-led",         note: "Palapye growth corridors" },
...
// ─── Baseline risk anchors ───────────────────────────────────────────────────
// Per-crop calibration anchors. Live Open-Meteo / MODIS / market data adjusts
// these ±30 in the risk engine. Pest is STATIC_PENDING_FAMEWS_API.
// FUTURE: integrate FAMEWS / FAO pest alert API

export const BASELINE_CROP_RISK = {
...
// ─── Data twin (regional proxy inference) ──────────────────────────────────────
// Used when local Botswana telemetry is sparse. Limpopo RSA semi-arid proxy —
// clearly labelled inferred, never mixed silently with live pipeline data.
...
export const TWIN_FIELD_INTEL = {
  ...
  ndviSource: "proxy",
  traffic: {
    name: "Node 104 (Limpopo proxy)",
    speed: 65,
    status: "green",
    msg: "Route clear (inferred)",
  },
};

/** Baseline risk anchors × district multiplier — data-twin mode (no live API). */
export function computeTwinRisk(district, crop) {
  const base = BASELINE_CROP_RISK[crop] ?? BASELINE_CROP_RISK.tomato;
  const mult = DISTRICT_RISK_MULT[district] ?? 1;
  const clamp = (v) => Math.round(Math.max(10, Math.min(95, v * mult)));
  return {
    district,
    crop,
    soil_score: clamp(base.soil),
    climate_score: clamp(base.climate),
    logistics_score: clamp(base.logistics),
    pest_score: clamp(base.pest),
    market_score: clamp(base.market),
    source: "twin",
    isInferred: true,
  };
}
...
// ─── Forecast engine ─────────────────────────────────────────────────────────
// Algorithm — static until empirically calibrated.
// WATER_M / INVEST_M multipliers: agronomic constants, calibrated manually.
...
// ─── Roadmap phases (internal strategy — static) ─────────────────────────────
...
export const ROADMAP_PHASES = [
  ...
  },
  {
    ...
    desc: "This dashboard. A live data pipeline, risk scoring model, and WhatsApp/USSD flow. A user should understand the recommendation in under 2 minutes.",
    deliverables: ["Dashboard (this app)", "Risk model v1", "Live data pipeline", "USSD flow design"],
...
export const DATA_SOURCES = [
  { layer: "1", layerLabel: "Open global",    name: "Open-Meteo",              freq: "Every 6h",        access: "open",    desc: "Live forecast, soil moisture, evapotranspiration per district" },
  { layer: "1", layerLabel: "Open global",    name: "NASA MODIS NDVI",         freq: "Weekly",          access: "open",    desc: "Satellite vegetation vigour (250m pixel) with ET-proxy fallback" },
  { layer: "1", layerLabel: "Open global",    name: "FAO FAOSTAT",             freq: "Weekly refresh",  access: "open",    desc: "National crop production baselines — demand-side anchor" },
  { layer: "1", layerLabel: "Open global",    name: "OSM Overpass",            freq: "Daily / weekly",  access: "open",    desc: "Live road network nodes and water sources across Botswana" },
  ...
  { layer: "3", layerLabel: "Proprietary",    name: "Farmer intention data",   freq: "Real-time",       access: "prop",    desc: "Planting intentions: crop, district, hectares, market target" },
